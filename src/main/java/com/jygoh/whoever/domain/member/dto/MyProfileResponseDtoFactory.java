...
public class MyProfileResponseDtoFactory {
    ...
    public MyProfileResponseDto createFromMember(Member member) {
        // 작성한 게시글 조회
        List<MyProfileResponseDto.PostForProfileDto> posts = postRepository.findAllByAuthorId(member.getId()).stream()
                .map(post -> new MyProfileResponseDto.PostForProfileDto(post.getId(), post.getTitle()))
                .collect(Collectors.toList());

        // 작성한 댓글 조회
        List<MyProfileResponseDto.CommentForProfileDto> comments = commentRepository.findByAuthorId(member.getId()).stream()
                .map(comment -> new MyProfileResponseDto.CommentForProfileDto(
                        comment.getId(),
                        comment.getContent(),
                        new MyProfileResponseDto.CommentForProfileDto.PostForCommentDto(
                                comment.getPostId(),
        ...
        // 팔로우하는 사용자 조회
        List<FollowResponseDto> following = followRepository.findById_FollowerId(member.getId()).stream()
                .map(follow -> {
